...
Not convinced? Please read the [Immutable Whitepaper](https://immutablesoft.org/whitepaper/) for more information.
...
![image metamask](./images/MetaMaskConnect.PNG)
...
![image nonetwork](./images/NoEthereumNetwork.PNG)
...
![image registeredMenu](./ImmutableRegisteredProduct.PNG)
...
![image productDetails](./images/ImmutableRegisteredProductLicenseOffer.PNG)
...
![image changeBank](./images/ImmutableChangeBank.PNG)
...
![image entityMove](./images/ImmutableEntityMove.PNG)

The second step is to use the new Ethereum address and access the Immutable Ecosystem. This address should be unregistered and on the Configure page is a button to Accept Entity Move. Press this Accept the Entity Move button and then enter the old address and press the Move button. Once the new Ethereum address has completed this second step the new address will be recognized by the Ecosystem as the Entity Administrator, and any tokens will be transferred if selected in the fist step.
 
![image entityAcceptMove](./images/ImmutableEntityAcceptMove.PNG)

With this two-step process, if there is a mistake with Step 1 then it can be resolved by the Entity. Only once an Entity has demonstrated they have possession of both Ethereum addresses (and associated keys) will an Entity address by changed. Step 1 can be repeated as many times as necessary and can even be performed ahead of time as a managerial escape hatch. However, if tokens are transferred to escrow as part of Step 1, they remain in escrow until Step 2 is completed. The tokens in Entity escrow are correctly maintained even if multiple transactions for Step 1 are performed.
...
Note the following details. The to: parameter is the address of the immutableLicense smart contract deployed on the Ethereum network. If the example above this is the Ropsten testnet address of the smart contact. This address will be different on mainnet. The Infua node also includes a URL specific to Ropsten, this will change to mainnet for production code. Finally the data field is the most difficult to understand. The first 4 bytes is the function hash identifier for the smart contract licenseStatus() function. This function takes three uint256 parameters, the Entity id, Product id and activation Identifier. If we split up the data field as below it is easier to understand how the data field is created. In the simplified example below the Entity Id is 2, product Id is 0 and Activation Id is 1.
...
For a production product the contract address, function hash, Entity Id and Product Id are all static and will be unchanged for the lifetime of the product. The unchanged fields can be hard coded into the application, however the Activation ID will be generated by the installed digital product and will have to be programatically added to the curl HTTP request. The Immutable toolkit will perform the secure steps necessary to generate a unique identifier locked to a specific hardware system and feed this identifier to the curl requests as well as when generating an activation function link within the application. There are more details and coding examples of how to build the Immutable toolkit into your digital product. See the toolkit for more information.